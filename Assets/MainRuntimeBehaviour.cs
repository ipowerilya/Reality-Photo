...
public class MainRuntimeBehaviour : MonoBehaviour
{
            ...
            try
            {
                ...
                if (data != null)
                {
                    ...
                    if (checkloading == false)
                    {
                        ...
                        txt.text = url;
                        StartCoroutine(downloadAllVideos(url, "videos.zip", true));
                    }
    ...
    IEnumerator downloadAllVideos(string videoUrl, string saveFileName, bool overwriteVideo)
    {
...
public class MainRuntimeBehaviour : MonoBehaviour
{
    ...
    IEnumerator downloadAllVideos(string videoUrl, string saveFileName, bool overwriteVideo)
    {
        ...
        if (persistantData.Contains(playbackDir) && !overwriteVideo)
        {
            ...
            //Play Video
            txt.text = "loaded";
            UnZipp(playbackDir, Path.Combine(Application.persistentDataPath, "unzipped"));
            //EXIT
            yield break;
        }
        else if (persistantData.Contains(playbackDir) && overwriteVideo)
